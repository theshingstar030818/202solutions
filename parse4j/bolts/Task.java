...
public class Task<TResult> {
	...
	private static final ExecutorService backgroundExecutor = Executors
			.newCachedThreadPool();
	/**
	 * An executor that runs a runnable inline (rather than scheduling it on a
	 * thread pool) as long as the recursion depth is less than MAX_DEPTH. If
	 * the executor has recursed too deeply, it will instead delegate to the
	...
	/**
	 * Creates a TaskCompletionSource that orchestrates a Task. This allows the
	 * creator of a task to be solely responsible for its completion.
	 * 
	...
	/**
	 * @return {@code true} if the task completed (has a result, an error, or
	...
	/**
	 * Makes a fluent cast of a Task's result possible, avoiding an extra
	 * continuation just to cast the type of the result.
	 */
	...
	/**
	 * Creates a task that completes when all of the provided tasks are
	 * complete.
	 */
	public static Task<Void> whenAll(Collection<? extends Task<?>> tasks) {
		...
		final Task<Void>.TaskCompletionSource allFinished = Task
				.<Void> create();
					...
					if (count.decrementAndGet() == 0) {
						if (errors.size() != 0) {
							if (errors.size() == 1) {
								...
								allFinished.setError(new AggregateException(
										errors));
							}
	...
	/**
	 * Continues a task with the equivalent of a Task-based while loop, where
	 * the body of the loop is a task continuation.
	 */
	...
	/**
	 * Adds a continuation that will be scheduled using the executor, returning
	 * a new task that completes after the continuation has finished running.
	 * This allows the continuation to be scheduled on different thread.
	 */
	public <TContinuationResult> Task<TContinuationResult> continueWith(
			final Continuation<TResult, TContinuationResult> continuation,
			final Executor executor) {
		boolean completed = false;
		final Task<TContinuationResult>.TaskCompletionSource tcs = Task
				.<TContinuationResult> create();
		synchronized (lock) {
	...
	/**
	 * Adds a synchronous continuation to this task, returning a new task that
	...
	/**
	 * Adds an Task-based continuation to this task that will be scheduled using
	 * the executor, returning a new task that completes after the task returned
	 * by the continuation has completed.
	 */
	...
	/**
	 * Adds an asynchronous continuation to this task, returning a new task that
		...
		return continueWithTask(
				new Continuation<TResult, Task<TContinuationResult>>() {
					public Task<TContinuationResult> then(Task<TResult> task) {
						if (task.isFaulted()) {
							return Task.<TContinuationResult> forError(task
									.getError());
	...
	/**
	 * Handles the non-async (i.e. the continuation doesn't return a Task)
	 * continuation case, passing the results of the given Task through to the
	 * given continuation and using the results of that call to set the result
	 * of the TaskContinuationSource.
	 * 
	 * @param tcs
	 *            The TaskContinuationSource that will be orchestrated by this
	 *            call.
	 * @param continuation
	 ...
	 * @param executor
	 *            The executor to use when running the continuation (allowing
	 *            the continuation to be scheduled on a different thread).
	 */
	private static <TContinuationResult, TResult> void completeImmediately(
			...
			final Continuation<TResult, TContinuationResult> continuation,
			final Task<TResult> task, Executor executor) {
		executor.execute(new Runnable() {
			public void run() {
				try {
	...
	/**
	 * Handles the async (i.e. the continuation does return a Task) continuation
	 * case, passing the results of the given Task through to the given
	 * continuation to get a new Task. The TaskCompletionSource's results are
	 * only set when the new Task has completed, unwrapping the results of the
	 * task returned by the continuation.
	 * 
	 * @param tcs
	 *            The TaskContinuationSource that will be orchestrated by this
	 *            call.
	...
	/**
	 * Allows safe orchestration of a task's completion, preventing the consumer
	 * from prematurely completing the task. Essentially, it represents the
	 * producer side of a Task<TResult>, providing access to the consumer side
	 * through the getTask() method while isolating the Task's completion
	...
	public class TaskCompletionSource {
		...
		/**
		 * Sets the cancelled flag on the Task if the Task hasn't already been
		 * completed.
		 */
		...
		/**
		 * Sets the result on the Task if the Task hasn't already been
		 * completed.
		 */
		...
		/**
		 * Sets the cancelled flag on the task, throwing if the Task has already
		 * been completed.
		 */
		...
		/**
		 * Sets the result of the Task, throwing if the Task has already been
		 * completed.
		 */
		...
		/**
		 * Sets the error of the Task, throwing if the Task has already been
		 * completed.
		 */
